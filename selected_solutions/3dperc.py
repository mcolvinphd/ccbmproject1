...
grid=[[[0 for x in range(side)] for y in range(side)] for z in range(side)]
while density < 0.8:  #Loop over densities
    perc=0
    for trial in range(trials):
        count=0
        #Randomly fill grid with barriers at specified density
        grid=[[[choices((0,1),weights=(1-density,density))[0] for x in range(side)] for y in range (side)] for z in range(side)]
        x,y,z = side//2,side//2,side//2 # Start particle at center
        for time in range(maxtime):
            # Randomly move particle
            sx,sy,sz = choice(steps)
            nx = x+sx
            ny = y+sy
            nz = z+sz
            ...
            if grid[nx][ny][nz]==1:
                continue
            if nx==0 or ny==0 or nz==0 or nx==(side-1) or ny==(side-1) or nz==(side-1):
                perc+=1
                break
            x=nx
            y=ny
